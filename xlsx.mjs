...
const emptyUploadsDirectory = () => {
  ...
  fs.readdir(uploadsSource, (err, files) => {
    ...
    if (err)
      return console.log("ERROR WHILE EMPTYING UPLOADS DIRECTORY: ", err);
...
const validateRow = async (row, rowCounter, sp) => {
  ...
  if (
    ...
    throw new Error(`La columna ${missingColumn} no puede estar vacía`, {
      cause: rowCounter,
    });
  }
